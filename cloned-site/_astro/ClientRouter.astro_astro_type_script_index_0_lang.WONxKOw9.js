import { s as S, n as g } from "./router.B-sij-_X.js";
const y = new Set(),
  s = new WeakSet();
let l,
  b,
...
function A() {
  for (const e of ["touchstart", "mousedown"])
    document.body.addEventListener(
      e,
      (t) => {
        i(t.target, "tap") && f(t.target.href, { ignoreSlowConnection: !0 });
      },
      { passive: !0 },
    );
}
...
function H() {
  ...
  u(() => {
    for (const t of document.getElementsByTagName("a"))
      s.has(t) || (i(t, "viewport") && (s.add(t), (e ??= M()), e.observe(t)));
  });
}
function M() {
  const e = new WeakMap();
      ...
      o.isIntersecting
        ? (a && clearTimeout(a),
          e.set(
            r,
            setTimeout(() => {
              (n.unobserve(r), e.delete(r), f(r.href));
            }, 300),
          ))
...
function P(e, t) {
  if (!navigator.onLine || (!t && w())) return !1;
  try {
...
(S || U() !== "none") &&
  ...
  document.addEventListener("submit", (e) => {
    ...
    const n = t,
      o = e.submitter,
      r = new FormData(n, o),
      a = typeof n.action == "string" ? n.action : n.getAttribute("action"),
      E = typeof n.method == "string" ? n.method : n.getAttribute("method");
    let c = o?.getAttribute("formaction") ?? a ?? location.pathname;
    const d = o?.getAttribute("formmethod") ?? E ?? "get";
    if (d === "dialog" || location.origin !== new URL(c, location.href).origin)
      return;
    const m = { sourceElement: o ?? n };
    if (d === "get") {
      const L = new URLSearchParams(r),
        h = new URL(c);
      ((h.search = L.toString()), (c = h.toString()));
    } else m.formData = r;
    (e.preventDefault(), g(c, m));
  }),
  T({ prefetchAll: !0 }));
