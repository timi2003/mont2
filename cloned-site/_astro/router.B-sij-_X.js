...
function _(t) {
    for (const e of document.scripts)
        for (const n of t.scripts)
            if (!n.hasAttribute("data-astro-rerun") && (!e.src && e.textContent === n.textContent || e.src && e.type === n.type && e.src === n.src)) {
                n.dataset.astroExec = "";
                break
            }
}
function $(t) {
    const e = document.documentElement
      , n = [...e.attributes].filter( ({name: o}) => (e.removeAttribute(o),
    o.startsWith("data-astro-")));
    [...t.documentElement.attributes, ...n].forEach( ({name: o, value: r}) => e.setAttribute(o, r))
}
function q(t) {
...
function B(t, e) {
    ...
    for (const n of e.querySelectorAll(`[${y}]`)) {
        ...
        r && (r.replaceWith(n),
        r.localName === "astro-island" && j(n) && !V(n, r) && (n.setAttribute("ssr", ""),
...
const W = () => {
  ...
  , E = ({activeElement: t, start: e, end: n}) => {
    t && (t.focus(),
    (t instanceof HTMLInputElement || t instanceof HTMLTextAreaElement) && (typeof e == "number" && (t.selectionStart = e),
  ...
  , V = (t, e) => t.getAttribute("props") === e.getAttribute("props")
  , K = t => {
    _(t),
    $(t),
    q(t);
    const e = W();
    B(t.body, document.body),
    e()
}
  , z = "astro:before-preparation"
  , G = "astro:after-preparation"
  , J = "astro:before-swap"
  , Q = "astro:after-swap"
  , Z = t => document.dispatchEvent(new Event(t));
...
class et extends N {
    ...
    swap;
    constructor(e, n) {
        super(J, void 0, e.from, e.to, e.direction, e.navigationType, e.sourceElement, e.info, e.newDocument, e.signal),
        this.direction = e.direction,
        this.viewTransition = n,
        this.swap = () => K(this.newDocument),
        Object.defineProperties(this, {
            direction: {
                enumerable: !0
            },
...
async function nt(t, e, n, o, r, i, u, a, l) {
    const d = new tt(t,e,n,o,r,i,window.document,u,a,l);
    return document.dispatchEvent(d) && (await d.loader(),
    d.defaultPrevented || (Z(G),
    d.navigationType !== "traverse" && R({
        scrollX,
        scrollY
    }))),
    d
...
const rt = history.pushState.bind(history)
...
let I, T = 0;
history.state ? (T = history.state.index,
scrollTo({
...
function ct(t) {
    ...
    for (const n of t.querySelectorAll("head link[rel=stylesheet]"))
        if (!document.querySelector(`[${P}="${n.getAttribute(P)}"], link[rel=stylesheet][href="${n.getAttribute("href")}"]`)) {
...
async function M(t, e, n, o, r) {
    ...
    F(e, n) && (t !== "back" && n.hash || t === "back" && e.hash)) {
        H(n, e, o, document.title, r),
        i === f && (f = void 0);
        return
    }
    const a = await nt(e, n, t, u, o.sourceElement, o.info, i.controller.signal, o.formData, l);
    ...
    async function l(s) {
        ...
        if (s.formData) {
            h.method = "POST";
            const b = s.sourceElement instanceof HTMLFormElement ? s.sourceElement : s.sourceElement instanceof HTMLElement && "form"in s.sourceElement ? s.sourceElement.form : s.sourceElement?.closest("form");
            h.body = b?.attributes.getNamedItem("enctype")?.value === "application/x-www-form-urlencoded" ? new URLSearchParams(s.formData) : s.formData
        }
        const m = await it(w, h);
        if (m === null) {
            s.preventDefault();
            return
        }
        if (m.redirected) {
            const b = new URL(m.redirected);
    ...
    if (document.documentElement.setAttribute(D, a.direction),
    O)
        c.viewTransition = document.startViewTransition(async () => await k(a, o, c, r));
    else {
        const s = (async () => {
...
function ut(t) {
    ...
    const e = history.state
      ...
      , o = n > T ? "forward" : "back";
    T = n,
    M(o, g, new URL(location.href), {}, e)
}
const L = () => {
    history.state && (scrollX !== history.state.scrollX || scrollY !== history.state.scrollY) && R({
        scrollX,
        scrollY
    })
}
...
{
    if (O || C() !== "none")
        if (g = new URL(location.href),
        addEventListener("popstate", ut),
        ...
        "onscrollend"in window)
            ...
            addEventListener("scroll", () => {
                t === void 0 && (o = history.state?.index,
                e = scrollY,
                n = scrollX,
